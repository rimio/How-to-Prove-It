...
\hfill
\begin{enumerate}
	\item We start by writing out the logical form of the statement
	$x \in \bigcup{i \in I} (A_i \cup B_i)$.
	\begin{align*}
		\exists i \in I (x \in A_i \vee x \in B_i)
		&= \exists i [i \in I \wedge (x \in A_i \vee x \in B_i)] \\
		&= \exists i [(i \in I \wedge x \in A_i) \vee (i \in I \wedge x \in B_i)] \\
		&= \exists i (i \in I \wedge x \in A_i) \vee \exists i (i \in I \wedge x \in B_i)
	\end{align*}
	Note that this last equivalence comes from Exercise 2.2.6, in which we proved that the
	existential quantifier distributes over disjunction.
	We then continue the chain of equivalences from the last statement
	\begin{align*}
		\exists i \in I (x \in A_i) \vee \exists i \in I (x \in B_i)
		&= \left( x \in \bigcup_{i \in I} A_i \right)
			\vee \left( x \in \bigcup_{i \in I} B_i \right) \\
	...
	\item We start by writing out the logical form of the statement
	 ...
	 \begin{align*}
	 	\left( x \in \bigcap \mathcal{F} \right) \wedge \left( x \in \bigcap \mathcal{G} \right)
	 	&= [\forall A \in \mathcal{F} (x \in A)] \wedge [\forall A \in \mathcal{G} (x \in A)] \\
	 	&= [\forall A (A \in \mathcal{F} \rightarrow x \in A)] \wedge
	 		[\forall A (A \in \mathcal{G} \rightarrow x \in A)] \\
		&= \forall A [(A \in \mathcal{F} \rightarrow x \in A) \wedge 
			(A \in \mathcal{G} \rightarrow x \in A)] \\
		&= \forall A [(A \in \mathcal{F} \vee A \in \mathcal{G}) \rightarrow x \in A]
	 \end{align*}
	 Note that this last equivalence comes from Part (1) of Exercise 1.5.7,
	 in which we proved that $(P \rightarrow R) \wedge (Q \rightarrow R)$ is equivalent to
	 $(P \vee Q) \rightarrow R$.
	 We then continue the chain of equivalences from the last statement.
	 \begin{align*}
	 	\forall A (A \in \mathcal{F} \cup \mathcal{G} \rightarrow x \in A)
	 	&= \forall A \in \mathcal{F} \cup \mathcal{G} (x \in A) \\
	...
	\begin{align*}
		...
		&= \forall i [i \in I \rightarrow (x \in A_i \wedge x \notin B_i)] \\
		&= \forall [(i \in I \rightarrow x \in A_i) \wedge (i \in I \rightarrow x \notin B_i)]
	\end{align*}
	Not that this last equivalence comes from Exercise 1.5.12,
	in which we proved that $P \rightarrow (Q \wedge R)$ is equivalent to
	$(P \rightarrow Q) \wedge (P \rightarrow R)$.
	We then continue the chain of equivalences from the last statement.
	\begin{align*}
		\forall i (i \in I \rightarrow x \in A_i) \wedge \forall i (i \in I \rightarrow x \notin B_i)
		&= \forall i \in I (x \in A_i) \wedge \forall i (i \notin I \vee x \notin B_i) \\
		&= \left( x \in \bigcap_{i \in I} A_i \right) \wedge \forall i \neg (i \in I \wedge x \in B_i) \\
		&= \left( x \in \bigcap_{i \in I} A_i \right) \wedge \neg \exists i (i \in I \wedge x \in B_i) \\
		&= \left( x \in \bigcap_{i \in I} A_i \right) \wedge \neg \exists i \in I (x \in B_i) \\
		&= \left( x \in \bigcap_{i \in I} A_i \right) \wedge \left( x \notin \bigcup_{i \in I} B_i \right) \\
...
\begin{proof}
We first note that $A_{1, 3} = \{ 1, 3, 4 \}$, $A_{1, 4} = \{ 1, 4, 5 \}$,
$A_{2, 3} = \{ 2, 3, 5 \}$, and $A_{2, 4} = \{ 2, 4, 6 \}$.
\begin{enumerate}
	\item We start from the definition of $B_j = \bigcup_{i \in I} A_{i, j}$.
	\begin{align*}
		B_3 &= A_{1, 3} \cup A_{2, 3}
			= \{ 1, 3, 4 \} \cup \{ 2, 3, 5 \} = \{ 1, 2, 3, 4, 5 \} \\
		B_4 &= A_{1, 4} \cup A_{2, 4}
			= \{ 1, 4, 5 \} \cup \{ 2, 4, 6 \} = \{ 1, 2, 4, 5, 6 \}
	\end{align*}
	
	\item Since $B_3 = \{ 1, 2, 3, 4, 5 \}$ and $B_4 = \{ 1, 2, 4, 5, 6 \}$,
	we have $\bigcap_{j \in J} B_j = \{ 1, 2, 4, 5 \}$.
	In other words, $\bigcap_{j \in J} \left( \bigcup_{i \in I} A_{i, j} \right) = \{ 1, 2, 4, 5 \}$.
	
	\item We follow the hint and perform the computation in two steps.
	\begin{align*}
		\bigcup_{i \in I} \left( \bigcap_{j \in J} A_{i, j} \right)
		&= \bigcup_{i \in I} (A_{i, 3} \cup A_{i, 4}) \\
		&= (A_{1, 3} \cap A_{1, 4}) \cup (A_{2, 3} \cap A_{2, 4}) \\
		&= \{ 1, 4 \} \cup \{ 2 \} \\
		&= \{ 1, 2, 4 \}
	\end{align*}
	We see that $\bigcap_{j \in J} \left( \bigcup_{i \in I} A_{i, j} \right) \neq
	\bigcup_{i \in I} \left( \bigcap_{j \in J} A_{i, j} \right)$.
	
	\item We first write out the logical form of 
	$x \in \bigcap_{j \in J} \left( \bigcup_{i \in I} A_{i, j} \right)$.
	\begin{equation*}
		\forall j \in J \exists i \in i (x \in A_{i, j})
	\end{equation*}
	Note that this statement says that for every $j \in J$,
	there is some $i \in I$, which may depend on the value of $j$,
	such that $x \in A_{i, j}$.
	In this situation, there are potentially multiple distinct $i$ for each $j$.
	To compare, we now write out the logical form of
	$x \in \bigcup_{i \in I} \left( \bigcap_{j \in J} A_{i, j} \right)$.
	\begin{equation*}
		\exists i \in I \forall j \in J (x \in A_{i, j})
	\end{equation*}
	Note that this statement says that there exists at least one $i \in I$
	such that $x \in A_{i, j}$ for all $j \in J$.
	In this situation, the $i$ value that exists works for all values of $j$.
	As we saw in Part (3), the two statements are not equivalent.
\end{enumerate}
\end{proof}
...
\hfill
\begin{enumerate}
	\item We start by writing down the logical form of the statement $x \in \bigcup \mathcal{F}$.
	\begin{equation*}
		\exists A \in \mathcal{F} (x \in A) = \exists A (A \in \mathcal{F} \wedge x \in A)
	\end{equation*}
	When $\mathcal{F} = \varnothing$, the statement $A \in \mathcal{F}$ is always false.
	Thus, when $\mathcal{F}$ is empty the statement $A \in \mathcal{F} \wedge x \in A$ is false
	regardless of the choice of $x$.
	From this, we conclude that $\bigcup \varnothing = \varnothing$.
	
	\item We start by writing down the logical form of the statement $x \in \bigcap \mathcal{F}$.
	\begin{equation*}
		\forall A \in \mathcal{F} (x \in A)
		= \forall A (A \in \mathcal{F} \rightarrow x \in A)
	\end{equation*}
	When $\mathcal{F}$ is empty, the statement $A \in \mathcal{F}$ is always false.
	Thus, the statement $A \in \mathcal{F} \rightarrow x \in A$ is true
	regardless of the choice of $x$.
\end{enumerate}
\end{proof}
